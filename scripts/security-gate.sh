...
if git grep -I -n -E -- 'define[[:space:]]*\([[:space:]]*['"'"'"]ATORA_DEV_MODE['"'"'"][[:space:]]*,[[:space:]]*true' -- '*.php' >/tmp/atora-security-gate-dev-mode.txt; then
	fail "ATORA_DEV_MODE está forzado a true:
$(cat /tmp/atora-security-gate-dev-mode.txt)"
fi
