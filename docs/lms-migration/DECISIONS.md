...
## D-007 · Estrategia de rollback del cutover
**Estado:** DECIDIDA — 2026-06-17

**Decisión:**
- **Disparadores de rollback (cualquiera):** una sola divergencia en un lector crítico
  post-cutover (`pc_*`); un incidente de acceso/matrícula reportado y confirmado; o un
  pico anómalo en el log de errores atribuible a la ruta de lectura de tablas.
- **Acción de rollback (inmediata, sin deploy):** `atora_lms_read_source = 'legacy'`.
  Como `atora_lms_dualwrite` se mantuvo activo, usermeta está al día y la reversión es
  instantánea y sin pérdida.
- **Quién decide:** el responsable técnico del despliegue; el rollback se ejecuta
  primero y se diagnostica después (preferir disponibilidad del alumno sobre depurar
  en caliente).
- **Tras un rollback:** corregir la causa, dejarlo verificado en el panel de paridad
  (volver a 0 divergencias PC), y **reiniciar la ventana de vigilancia** antes de
  reintentar el flip. No se reintenta el mismo día.
- **Ventana de vigilancia antes de F5:** **14 días** estables post-cutover (0
  divergencias PC, dualwrite activo) antes de congelar legacy.

**Consecuencias:** F5 ("congelar legacy" = apagar `atora_lms_dualwrite` + quitar
listeners del compat layer) **no** se aborda hasta cerrar la ventana de D-007 sin
rollbacks.
...
### F4 — Cutover de lectura (2026-06-17)

| Tarea | Archivos | Estado |
|---|---|---|
| F4.0 — Checklist pre-flight (gate D-006 + requisitos) | `docs/lms-migration/F4-cutover-log.md` | ✅ Creado |
| F4.1 — Shadow-read bidireccional (pc_* readers) | `modules/lms/class-lms-parity.php`, `includes/helper/trait-helper-academic.php` | ✅ |
| F4.2 — Toggle flip/rollback en panel admin | `modules/lms/class-lms-migration-admin.php`, `views/migration-admin.php` | ✅ |
| F4.3 — Panel post-cutover: divergencias PC, días estables, gate F5 | `modules/lms/views/migration-admin.php` | ✅ |
| D-007 — Estrategia de rollback documentada | `docs/lms-migration/DECISIONS.md` | ✅ |
| Versión → 6.1.0 | `atora_lms.php` | ✅ |
| Flag en `legacy` — F4.2 flip NO se ejecuta automáticamente | — | ✅ Requiere gate humano |
...
| 2026-06-17 | D-007 | Decidida: rollback inmediato a legacy ante 1 divergencia PC; 14 días estables antes de F5 |
| 2026-06-17 | F4.1  | Shadow-read bidireccional: 5 métodos `shadow_*_pc()` en `LMS_Parity`; 5 lectores canónicos branched on `is_tables()` |
| 2026-06-17 | F4.2  | Toggle admin `atora_lms_toggle_read_source` + botón flip/rollback en panel migración |
| 2026-06-17 | F4.3  | Panel post-cutover: divergencias PC, días estables, gate F5; versión → 6.1.0 |
