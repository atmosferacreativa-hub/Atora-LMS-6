	...
	<div class="tp-stats__inner">
		<dl class="tp-stats__grid">
			<?php if ( $data['published_courses_n'] > 0 ) : ?>
				<div class="tp-stat">
					<dt class="tp-stat__value"><?php echo esc_html( number_format_i18n( $data['published_courses_n'] ) ); ?></dt>
					<dd class="tp-stat__label"><?php echo esc_html( _n( 'Curso', 'Cursos', $data['published_courses_n'], 'atora-lms' ) ); ?></dd>
				</div>
			<?php endif; ?>
			<?php
			// Estudiantes: suma de inscritos en todos sus cursos (si CLMS_Enrollment existe)
			$students = 0;
			if ( class_exists( 'CLMS_Enrollment' ) && ! empty( $data['published_courses'] ) ) {
