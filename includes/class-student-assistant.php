...
class CLMS_Student_Assistant {
	...
	protected function get_client_ip() {
		...
		// Nunca guardamos la IP en claro dentro de transients.
		return hash( 'sha256', $ip . '|' . wp_salt( 'auth' ) );
	}

	/**
	 * Resuelve IP de cliente respetando proxies confiables.
	 * Solo usa cabeceras reenviadas cuando REMOTE_ADDR coincide con un proxy de confianza.
	 */
	protected function resolve_client_ip(): string {
		$remote_addr = $this->normalize_ip_candidate( isset( $_SERVER['REMOTE_ADDR'] ) ? wp_unslash( $_SERVER['REMOTE_ADDR'] ) : '' );
		if ( '' === $remote_addr ) {
			return '';
		}

		if ( ! $this->is_trusted_proxy_ip( $remote_addr ) ) {
			return $remote_addr;
		}
		...
		foreach ( $forwarded_headers as $header_key ) {
			...
			$candidate = $this->extract_forwarded_ip( (string) $raw );
			if ( '' !== $candidate ) {
				return $candidate;
			}
		}
	 ...
	 */
	protected function extract_forwarded_ip( string $raw ): string {
		...
		foreach ( $candidates as $candidate ) {
			...
			if ( ! $this->is_trusted_proxy_ip( $ip ) ) {
				return $ip;
			}
		}
	 ...
	 */
	protected function is_trusted_proxy_ip( string $ip ): bool {
		if ( '' === $ip || ! filter_var( $ip, FILTER_VALIDATE_IP ) ) {
			return false;
		}

		$trusted = apply_filters(
			'clms_student_assistant_trusted_proxies',
			array(
				'127.0.0.1',
				'::1',
				'10.0.0.0/8',
	 ...
	 */
	protected function normalize_ip_candidate( $value ): string {
